				...
				<form method="POST" action="{{route('admin.board.updateViewBoard.store')}}">
                    ...
                    <div class="form-group">
                        ...
                        <select name="reftype" id="" class="form-control">
                            ...
                            <option value="1" {{ $data[0]['reftype'] == 'Ecollector' ? "selected" : ""}}>Ecollector</option>
                            <option value="2" {{ $data[0]['reftype'] == 'MBS Publikasi' ? "selected" : ""}}>MBS Publikasi</option>
                            <option value="3" {{ $data[0]['reftype'] == 'MBS Otorisasi' ? "selected" : ""}}>MBS Otorisasi</option>                       
                        </select>
                    </div>

                    <div class="form-group">
                        <label for="">Message</label>
                        <input type="textarea" name="news" value="{{$data[0]['news']}}" class="form-control">
                    </div>

                    <div class="form-group">
                        <label for="">Tanggal</label>
                        <input type="date" name="refdate" value="{{ \Carbon\Carbon::parse($data[0]['refdate'])->format('Y-m-d')}}"  class="form-control">
